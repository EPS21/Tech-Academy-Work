...
namespace ScheduleUsers.Controllers
{
    ...
    public class WorkTimeEventController : Controller
    {
        ...
        // This is the complete copy of above why is this here
        ...
        public ActionResult Create(LoginViewModel workTimeEvent)    // workTimeEvent grabs the login email, password, and remember me
        {
            ...
            if (notFinishedEvent != null)
            {
                ...
                // Add a viewstate variable something to determine the clockout message
                ViewBag.Message = "Clock out successful";
            ...
            else
            {
                ...
                // Add a viewstate variable something to determine the clockin message
                ViewBag.Message = "Clock in successful";

                // get rid of modelstate error after we have some other confirmation take place
        ...
        public JsonResult CheckForUserData(string userdata) // userdata comes from function UserCheck() on Login.cshtml
        {
            ...
            if (searchData != null)
            {
                ...
                // clock in from a previous day and todaysWorkTimeEvents count is nothing (because today has passed)
                if (todaysWorkTimeEvents.Count == 0 && notFinishedEvent != null)
                {
                    return Json(new WorkTimeEventCreateViewModel(2, notFinishedEvent, newMessages), JsonRequestBehavior.AllowGet);
                }

                // if todaysWorkTimeEvents exists, AND a notFinishedEvent exists (still clocked)
                else if (todaysWorkTimeEvents != null && notFinishedEvent != null)
                { 
